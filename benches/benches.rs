	...
	T::Error: std::fmt::Debug,
{
	c.bench_function_over_inputs(
		&format!("vec_encode_{}", type_name::<T>()),
		|b, &vec_size| {
	...
	c.bench_function_over_inputs(
		&format!("vec_decode_{}", type_name::<T>()),
		|b, &vec_size| {
			let vec: Vec<T> = (0..=127u8)
				.cycle()
				.take(vec_size)
...
fn encode_decode_complex_type(c: &mut Criterion) {
	...
	let complex_types = vec![
		ComplexType {
			_val: 3,
			_other_val: 345634635,
			_vec: vec![1, 2, 3, 5, 6, 7],
		},
		ComplexType {
			_val: 1000,
			_other_val: 0980345634635,
			_vec: vec![1, 2, 3, 5, 6, 7],
		},
		ComplexType {
			_val: 43564,
			_other_val: 342342345634635,
			_vec: vec![1, 2, 3, 5, 6, 7],
		},
	];
	...
	c.bench_function_over_inputs(
		"vec_encode_complex_type",
		move |b, &vec_size| {
			let vec: Vec<ComplexType> = complex_types
				.clone()
				.into_iter()
				.cycle()
				.take(vec_size)
	...
	c.bench_function_over_inputs(
		"vec_decode_complex_type",
		move |b, &vec_size| {
			let vec: Vec<ComplexType> = complex_types2
				.clone()
				.into_iter()
				.cycle()
				.take(vec_size)
...
fn bench_fn(c: &mut Criterion) {
	...
	c.bench_function(
		"vec_append_with_decode_and_encode",
		vec_append_with_decode_and_encode,
	);
	c.bench_function(
		"vec_append_with_encode_append",
		vec_append_with_encode_append,
	);
...
fn encode_decode_bitvec_u8(c: &mut Criterion) {
	...
	c.bench_function_over_inputs(
		...
		|b, &size| {
			let vec: BitVec = [true, false].iter().cloned().cycle().take(size).collect();
