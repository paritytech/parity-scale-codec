...
pub trait DecodeDedupLength {
	/// Return the number of elements in `self_encoded`.
	fn dedup_len(self_encoded: &[u8]) -> Result<usize, Error>;
}
...
macro_rules! impl_dedup_len {
    ( $( $type:ident< $($g:ident),* > ),* ) => { $(
        impl<$($g),*> DecodeDedupLength for $type<$($g),*>
        where
            $($g: Decode + Ord),*
        {
            fn dedup_len(mut self_encoded: &[u8]) -> Result<usize, Error> {
                let decoded: $type<$($g),*> = $type::<$($g),*>::decode(&mut self_encoded)?;
                let deduplicated: BTreeSet<_> = decoded.into_iter().collect();
                Ok(deduplicated.len())
            }
        }
    )*}
...
impl_dedup_len!(Vec<T>, BTreeSet<T>, BTreeMap<K, V>);
